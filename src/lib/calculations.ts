...
import type { Lead, OfferVariant } from '../services/data';
...
export function calculateROI(data: WizardData): ExtendedROICalculations {
  ...
  // Investition — Speicherkosten skalieren mit kWh (passt zu Step-4-Preisen: 500€/kWh + 2000€ Basis)
...
export function recalculateLead(lead: Lead): Partial<Lead> {
  ...
  const storageKwh = hasBattery ? 10 : 0; // Lead speichert keine kWh; 10 kWh als Standard
...
// ── Offer Variants: Speicher-Optionen ─────────────────────────────────

interface VariantConfig {
  key: 'einstieg' | 'optimal' | 'zukunftssicher';
  label: string;
  description: string;
  storageKwh: number;
  hasWallbox: boolean;
...
const VARIANT_CONFIGS: VariantConfig[] = [
  {
    key: 'einstieg',
    label: 'Einstieg',
    description: 'Ohne Speicher — ideal für hohen Eigenverbrauch',
    storageKwh: 0,
    hasWallbox: false,
    isRecommended: false,
  },
  {
    key: 'optimal',
    label: 'Optimal',
    description: 'Mit 10 kWh Speicher — beste ROI-Kombination',
    storageKwh: 10,
    hasWallbox: false,
    isRecommended: true,
  },
  {
    key: 'zukunftssicher',
    label: 'Zukunftssicher',
    description: '15 kWh Speicher + Wallbox — für E-Auto & Wärmepumpe',
    storageKwh: 15,
    hasWallbox: true,
    isRecommended: false,
  },
...
// Kosten pro kWh Speicher (inkl. Installation)
const STORAGE_COST_PER_KWH = 600;
...
function calculateVariantMetrics(
  ...
  // Eigenverbrauchsrate steigt mit Speichergröße
  const isCommercial = lead.building_type === 'gewerbe' || lead.building_type === 'firmengebaeude';
  let selfConsumptionRate = isCommercial
    ? (hasBattery ? 0.80 : 0.60)
    : (hasBattery ? 0.65 : 0.30);

  // Größerer Speicher = höhere Autarkie
  if (storageKwh >= 15) selfConsumptionRate = Math.min(0.95, selfConsumptionRate + 0.10);
  else if (storageKwh >= 10) selfConsumptionRate = Math.min(0.90, selfConsumptionRate + 0.05);
  ...
  // Investition: Basis + Speicher
  const baseInvestment = Math.round(kwp * INVEST_PER_KWP);
  const storageCost = hasBattery ? Math.round(storageKwh * STORAGE_COST_PER_KWH) : 0;
  const wallboxCost = hasWallbox ? 1200 : 0;
  const investment = baseInvestment + storageCost + wallboxCost;
  ...
  return { kwp, investment, annualSavings, amortization, autarky, profit20Years };
}

export function generateStorageVariants(lead: Lead): Omit<OfferVariant, 'id' | 'lead_id' | 'created_at'>[] {
  return VARIANT_CONFIGS.map((cfg) => {
    const calc = calculateVariantMetrics(lead, cfg.storageKwh, cfg.hasWallbox);
    return {
      variant_key: cfg.key,
      label: cfg.label,
      description: cfg.description,
      storage_kwh: cfg.storageKwh,
      has_wallbox: cfg.hasWallbox,
      has_backup: false,
      kwp: calc.kwp,
      ...
      price_eur: calc.investment,
      is_primary: cfg.key === 'einstieg',
